...
async def get_word_stats(
    ...
    Returns:
        {
          "total": int,
          "by_status": {"new": N, "learning": N, "practicing": N, "mastered": N},
          "by_cefr": {"A1": N, ..., "C2": N},
          "by_source": {"ocr": N, "chat": N, "lecture": N, "import": N, "manual": N},
          "streak_days": int,
        }
    """
    ...
    by_status: Dict[str, int] = {"new": 0, "learning": 0, "practicing": 0, "mastered": 0}
    for status_val, cnt in status_rows:
        if status_val in by_status:
            by_status[status_val] = cnt

    # ── by_cefr: JOIN words GROUP BY cefr_level ───────────────────
    cefr_rows = (
        await db.execute(
            select(
                Word.cefr_level,
    ...
    source_rows = (
        await db.execute(
            ...
            .where(UserWord.user_id == user_id)
            .group_by(UserWord.source)
        )
    ).all()

    by_source: Dict[str, int] = {
        "ocr": 0, "chat": 0, "lecture": 0, "import": 0, "manual": 0,
    }
    for src, cnt in source_rows:
        if src in by_source:
            by_source[src] = cnt

    # ── streak_days: consecutive days with word activity ──────────
    date_rows = (
        await db.execute(
            select(cast(UserWord.last_seen_at, Date))
            .where(UserWord.user_id == user_id)
            .distinct()
            .order_by(cast(UserWord.last_seen_at, Date).desc())
        )
    ).all()

    streak_days = _calculate_streak([row[0] for row in date_rows])
...
def _calculate_streak(active_dates: list) -> int:
    """Count consecutive active days backwards from the most recent date.

    Only counts as a streak if the most recent date is within 1 day of today.
    active_dates is sorted DESC.
    """
    ...
    # Streak is broken if the most recent activity is more than 1 day ago
    if most_recent < date.fromordinal(today.toordinal() - 1):
        return 0
