...
# ═══════════════════════════════════════════════════════════════════════
# Container / Service Management  (docker SDK via Unix socket)
# ═══════════════════════════════════════════════════════════════════════

_docker_client: Any = None

def _get_docker() -> Any:
    """Lazy-init Docker client via Unix socket."""
    if not HAS_DOCKER:
        return False
    global _docker_client
    if _docker_client is None:
        try:
            _docker_client = docker.DockerClient(base_url="unix:///var/run/docker.sock")  # type: ignore
            _docker_client.ping()
        except Exception:
...
async def list_containers(
    ...
    """List all Docker containers via Unix socket."""
    try:
        client = _get_docker()
        if not client:
            return {"code": 500, "message": "Docker socket not available — mount /var/run/docker.sock"}
        ...
        for c in client.containers.list(all=True):
            ...
            containers.append({
                ...
                "ports": ", ".join(
                    f"{k}:{v[0]['HostPort']}" for k, v in (c.attrs.get("NetworkSettings", {}).get("Ports", {}) or {}).items() if v
                ) if c.attrs.get("NetworkSettings", {}).get("Ports") else "",
                "created": c.attrs.get("Created", ""),
            })

        return {"code": 200, "data": {"containers": containers}}
    except Exception as e:
        return {"code": 500, "message": f"Docker access error: {str(e)}"}
...
async def manage_container(
    ...
    try:
        client = _get_docker()
        if not client:
            return {"code": 500, "message": "Docker not available"}
        container = client.containers.get(container_name)
        getattr(container, action)()
        return {"code": 200, "message": f"Container '{container_name}' {action}ed"}
    except Exception as e:
        return {"code": 500, "message": str(e)}
